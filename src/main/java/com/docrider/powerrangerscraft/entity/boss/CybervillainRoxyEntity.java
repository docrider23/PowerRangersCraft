...
public class CybervillainRoxyEntity extends BaseFootsoldierEntity {
    ...
    public CybervillainRoxyEntity(EntityType<? extends BaseFootsoldierEntity> type, Level level) {
        ...
        this.setItemSlot(EquipmentSlot.FEET, new ItemStack(BeastMorphersItems.ROXY_EVOX_BEAST_X_MORPHER.get()));
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(MobsItems.CYBERVILLAIN_ROXY_BLASTER_1.get()));
        this.setItemSlot(EquipmentSlot.OFFHAND, new ItemStack(MobsItems.CYBERVILLAIN_ROXY_BLASTER_2.get()));
    }
    ...
    public void actuallyHurt(DamageSource source, float amount) {
        ...
        if(!this.level().isClientSide() && source.getEntity() instanceof Player playerIn && this.getHealth()<25
                && this.getItemBySlot(EquipmentSlot.FEET).getItem()==BeastMorphersItems.ROXY_EVOX_BEAST_X_MORPHER.get()) {

            Inventory Inventory = playerIn.getInventory();
            boolean hasVisor = Inventory.countItem(BeastMorphersItems.BEAST_X_VISOR.get()) != 0;

            if (hasVisor) {
                this.setItemSlot(EquipmentSlot.FEET, new ItemStack(BeastMorphersItems.ROBO_ROXY_EVOX_BEAST_X_MORPHER.get()));
                this.bossEvent.setName(Component.translatable("entity.powerrangerscraft.cybervillain_robo_roxy").withStyle(ChatFormatting.GOLD));
    ...
    public void remove(RemovalReason p_149847_) {
        ...
        if ( this.isDeadOrDying()) {
            if(!this.level().isClientSide() && this.getItemBySlot(EquipmentSlot.FEET).getItem()==BeastMorphersItems.ROBO_ROXY_EVOX_BEAST_X_MORPHER.get())
            {
                Random generator = new Random();
                int randomInt = generator.nextInt(3);

                if (randomInt == 0) {
                    ItemEntity key = new ItemEntity(level(), getX(), getY(), getZ(), new ItemStack(BeastMorphersItems.ROBO_ROXY_MORPH_X_KEY.get(), 1), 0, 0, 0);
                    key.setPickUpDelay(0);
                    level().addFreshEntity(key);
                }
                else if (randomInt == 1) {
                    ItemEntity key = new ItemEntity(level(), getX(), getY(), getZ(), new ItemStack(BeastMorphersItems.BEAST_X_KING_MORPH_X_KEY.get(), 1), 0, 0, 0);
                    key.setPickUpDelay(0);
                    level().addFreshEntity(key);
                }
                else if (randomInt == 2) {
